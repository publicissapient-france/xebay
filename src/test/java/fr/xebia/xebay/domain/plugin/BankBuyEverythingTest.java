...
public class BankBuyEverythingTest {
    ...
    public void item_should_be_bought_by_bank_if_selling_price_is_same_as_item_value() {
        Item item = new Item("category", "name", new BigDecimal(4.3));
        BankBuyEverything plugin = new BankBuyEverything();

        boolean authorizeThisBidOfferToSell = plugin.authorizeIfActivated(new BidOfferToSell(item, new BigDecimal(4.3)));
    ...
    public void item_should_be_not_be_bought_by_bank_if_selling_price_is_different_than_item_value() {
        ...
        boolean authorizeThisBidOfferToSell = plugin.authorizeIfActivated(new BidOfferToSell(item, new BigDecimal(5)));
