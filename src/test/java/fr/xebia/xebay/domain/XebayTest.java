...
public class XebayTest {
    ...
    public void should_not_bid_with_less_than_ten_percent_of_initial_value() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(4.3))));
        expectedException.expect(BidException.class);
        expectedException.expectMessage("you have bid 4.72$ which is less than 4.73$ (current value 4.30$ + ten percent of initial value 4.30$)");

        bidEngine.bid(user, "an item", 4.3 + 0.43 - 0.01);
    }

    @Test
    public void should_bid_if_exactly_ten_percent_increment_of_initial_value() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(38.07))));
    ...
    public void a_bid_is_valid_until_bid_offer_expires() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(4.3)), new Item("category", "another item", new Amount(2.4))), () -> true);
    ...
    public void when_only_one_user_bids_then_he_wins() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(4.3)), new Item("category", "another item", new Amount(2.4))), expiration);
        bidEngine.bid(user, "an item", 4.3 + 0.7);

        resolvesBidOffer(bidEngine);

        assertThat(user.getBalance()).isEqualTo(new Amount(995));
        Item purchasedItem = user.getItems().iterator().next();
        assertThat(purchasedItem.getName()).isEqualTo("an item");
        assertThat(purchasedItem.getValue()).isEqualTo(new Amount(5));
    ...
    public void should_not_makes_offer_if_item_is_null() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(4.3))), expiration);
    ...
    public void should_not_makes_offer_if_user_is_null() {
        ...
        bidEngine.offer(null, new Item("category", "an item", new Amount(4.3)), 5.9);
    }
    ...
    public void when_a_bid_is_won_seller_increase_money() {
        ...
        assertThat(seller.getBalance()).isEqualTo(new Amount(1000.8));
    }

    @Test
    public void when_nobody_bid_then_item_value_loose_ten_percent_of_its_value() {
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", new Amount(4.33))), expiration);
    ...
    public void user_cant_bid_without_enough_balance() {
        Amount initialItemValue = User.INITIAL_BALANCE.add(new Amount(-100));
        BidEngine bidEngine = new BidEngine(new Items(new Item("category", "an item", initialItemValue), new Item("category", "another item", new Amount(5))));
        expectedException.expect(BidException.class);
